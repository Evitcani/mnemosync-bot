...
let PartyController = PartyController_1 = class PartyController {
    ...
    getByGuild(guildId) {
        return PartyController_1.getRepo().find({ where: { guildId: guildId } }).then((parties) => {
    ...
    getByNameAndGuild(partyName, guildId) {
        ...
        return PartyController_1.getRepo()
            ...
            .where("\"parties\".\"guild_id\" = :id AND LOWER(\"parties\".\"name\") LIKE LOWER('%:name%')", [guildId, sanitizedPartyName])
            .getMany()
            .then((parties) => {
